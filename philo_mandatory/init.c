...
void	init_table(t_table *table)
{
  ...
  table->philos_array = malloc_handle(sizeof(t_philo) * table->philosophers_number);
  table->forks_array = malloc_handle(sizeof(t_fork) * table->philosophers_number);
  while (++i < table->philosophers_number)
  {
    mutex_handle(&table->forks_array[i].mutex, INIT);
    table->forks_array[i].id = i; // id of the fork
  }
  init_philos(table);
}
